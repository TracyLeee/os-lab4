...
int prepend_history(phistory_t hist, const pcommand_t cmd)
{
    ...
    if (hist->length > 0) {
        ...
        #ifdef PATCH_1
            unsigned int cp_num = hist->length == MAX_CMD_HISTORY ? hist->length - 1 : hist->length;
            
            for (unsigned int i = cp_num; i > 0; --i) {
                ...
                if (get_variable(vars, &tok[1], &value) == 0) {
                    ...
                    /* Perform the move operation in `cmd`. */
                        ...
                        if (move_dst > move_src) {
                            char *move_tmp = malloc(strlen(*cmd) + 1 + move_dst - move_src);
